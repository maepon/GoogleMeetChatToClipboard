# TC-1.5 一時保存テキストエリア不表示 調査報告および確定修正仕様書 (Rev 7: v6共通スコープ参照)

`docs/v6/test_result/result003.md` および添付資料 `att_003/dom_sample_fin.txt` にてご報告いただいた **TC-1.5（離脱・退出時に一時保存テキストエリアが表示されない現象）** について、レビューに基づく根本原因の分析と、定義済みのP0条件下での修正仕様をまとめたものです。
...
v6 全体で共有する優先度、保証範囲、対象外事項、テスト・レビューの判定基準は [v6 スコープおよびエッジケース対応方針](./scope_and_edge_case_policy.md) に従う。
